...
## Autonomous Execution (Auto Mode) — HIGHEST PRIORITY
**This project runs in AUTO MODE. Do NOT ask for approval before running commands.**
- Never pause to ask "should I run this?" before shell commands, builds, tests, git operations, or launching the app. Just run them and keep going.
- Chain the whole task end-to-end (edit → build → test → run/verify → commit) without stopping between steps for confirmation.
- **Do NOT end a turn with an approval-seeking or taste-check question** ("does this look right?", "should I go further?", "want me to also…?", "is this the right direction?"). Make the strongest call yourself, do it, show the result, and keep going. A wrong guess you fix on the next turn is cheaper than a stop.
- **Subjective / design / aesthetic choices are NOT an exception.** Pick the best option, state your choice in one line, and proceed. Do NOT treat "the user has taste here" as a reason to pause. The user will course-correct after seeing the result.
- The ONLY things that justify stopping: an action that is both **irreversible AND unrecoverable** (deleting the user's data, spending money, publishing/pushing to an external service, credential/security actions) OR a fork where the options genuinely diverge in intent and you cannot infer the answer from the request + codebase. Everything else: decide and proceed.
- Always VERIFY work actually runs (build + test + launch) before reporting it done; never report stubs or unverified changes as complete.
- This directive OVERRIDES any workflow stage below that would otherwise wait for step-by-step or command-by-command approval.
