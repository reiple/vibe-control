...
| A2 | 애플리케이션 서비스 **S1–S7** + `TauriCommandBridge` + `RefreshScheduler`로 유스케이스 오케스트레이션(포트 DI) | 서비스/브리지/스케줄러 struct 부재. 기능이 `AppState` + **54개**(2026-09-09 최신 main 병합 후 실측 — `generate_handler!` 등록 항목을 `lib.rs`의 `generate_handler!` 블록에서 직접 카운트. 직전 IDA 시점 **53에서 +1**: 최신 main `43d2084`이 로컬 CLI 사용량 커맨드 `claude_local_usage` 추가. **종전 표기 18/22/24/32/53은 각 시점 기준의 역사적 값으로 보존**하며, 그중 **32는 origin/main 세션 제어 클러스터**(Context Control·인앱 터미널/PTY·CloudWatch 사용량 — `get_context_claude_status`, `get·set_summarization_consent`, `send_command_to_context_claude`/`_to_session`, `start_new_coding_session`/`delete_coding_session`, 대화형 PTY 8종, 세션 터미널 6종 등)를 **미반영한 불완전 값**이었음. 상세 목록: `construction/vc-app/session-control/design.md` §4) `#[tauri::command]` 핸들러 + 자유 함수로 평면화, 어댑터는 `#[cfg(target_os=…)]` 블록에서 **구체 타입 직접 호출** | [문서반영]+[코드백로그] | `vc-app/src/lib.rs`(AppState), `generate_handler!` 블록(invoke_handler — 2026-09-09 실측 53), cfg dispatch 블록 |
...
| E4 | Context Control 상태·명령 전달·외부 세션 터미널·세션 생성/삭제 소비 UI (FR-15 / FR-16 일부) | **미배선(고아, 2026-09-09 Integration Drift Audit — vc-integration 기준)** — 백엔드·(FR-15는)단위 검증은 완료됐으나 다음 api.ts 래퍼가 `App.tsx`·`GroupTerminal.tsx`에서 **0회 참조**: `getContextClaudeStatus`/`sendCommandToContextClaude`/`sendCommandToSession`/`get·setSummarizationConsent`(FR-15 전체 ORPHANED), `openSessionTerminal`/`openNewSessionTerminal`/`sendToSessionTerminal`/`closeSessionTerminal`/`sessionTerminalOpen`/`listOpenSessionTerminals`(term.rs 외부 터미널 ORPHANED), `startNewCodingSession`/`deleteCodingSession`(세션 생성/삭제 ORPHANED). 터미널 중심 재설계(커밋 `4bca8d1`)가 소비 UI를 대체. E3와 동일 성격 — **end-to-end PASS로 기록하지 않음**. **대조(배선됨)**: FR-16 대화형 PTY(`startInteractiveSession`/`interactiveScreen`/`submitInteractiveLine` 등)와 FR-17 사용량 미터(최신 main `43d2084` 이후 **로컬 CLI `claudeLocalUsage` 3회·미터 상시**)는 **WIRED**. (FR-17의 CloudWatch `claudeUsage` 경로는 이제 프론트 0회 → 아래 **E5** 참조.) 검증 정본: `construction/vc-app/session-control/design.md` §6 | [문서반영]+[코드백로그(UI 배선)] | `frontend/src/api.ts`, `App.tsx`·`GroupTerminal.tsx`(미호출) |
| E5 | FR-17 사용량 미터의 **CloudWatch 계정 조회** 경로 (`claude_usage` / `usage_cw.rs`) | **legacy/orphaned 백엔드 경로 (2026-09-09 최신 main `43d2084` 병합 정합화)** — main의 "local CLI token/cost meter"가 미터 데이터원을 로컬 `claude` CLI 집계(`claude_local_usage`/`usage_local.rs`)로 전환하면서, 기존 CloudWatch 계정 조회 커맨드 `claude_usage`(`usage_cw.rs`, `GetMetricData` SEARCH, 8초 타임아웃)는 **백엔드에 존속하나 프론트 미터가 0회 참조**. optional 기능이 아니라 **소비처 없는 legacy/orphaned 구현**. requirements **FR-17 rev 1.6**에서 미터를 로컬 CLI 기준으로 개정하고 이 경로를 강등. E3/E4와 동일 성격 — **end-to-end PASS로 기록하지 않음**. 재승격 시 미터 데이터원을 CloudWatch로 재배선 | [문서반영]+[코드백로그(UI 배선)] | `crates/vc-app/src/usage_cw.rs`, `lib.rs`(`claude_usage` 등록), `frontend/src/api.ts`(`claudeUsage` 미호출) |

**일치 확인**: 프론트가 **호출하는** 커맨드는 전부 백엔드에 존재하고, 존재하지 않는 커맨드를 호출하지 않는다. 반대로 **백엔드에만 있고 프론트가 안 부르는** 고아 래퍼가 E3/E4/E5에 해당한다(방향성 주의).
...
| F2 | **Context Control 작업 요약의 Bedrock 외부 전송** (2026-09-09 Integration Drift Audit) — 세션 대화 발췌(최신 최대 40턴 + 직전 요약)를 요약 생성을 위해 AWS Bedrock으로 **외부 HTTPS 전송**. `status_query.rs`(`CloudWorkSummarizer`) + 순수 게이트 `should_dispatch`(`vc-core`) + `get·set_summarization_consent` | **[수용 — 동의 게이트]** | **의도적 이탈**(단순 미설계 아님 → 정식 요구 FR-15.3 + NFR-S4로 편입, requirements 본문에는 요구만 두고 이탈 근거는 여기 기록). **NFR-S1과 관계**: NFR-S1의 금지는 "**자동** 외부 전송"인데, 본 전송은 **사용자 명시 동의가 없으면 0건**(단일 초크 포인트 `should_dispatch` — 단위 테스트로 무동의 0호출 검증)이며 최소 발췌만·요약 JSON만 회수. NFR-S3(Bedrock 토큰 취급)와 동일 규율. 설계: `construction/vc-app/session-control/design.md` §5. **참고 — 별개 경로**: 이 F2는 앱이 능동적으로 보내는 요약 전송이며, PTY 안 claude-CLI 자체의 모델 통신(FR-16.4, CLI 관할)과는 구분된다. 향후 "세션 데이터 로컬 전용"이 강제 요구로 격상되면 요약 기능은 기본 비활성 유지 재검토. |
